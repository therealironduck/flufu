...
func buildJokePrompt(model llama.Model) string {
	template := llama.ModelChatTemplate(model, "")
	if template == "" {
		template = "chatml"
	}
	...
	messages := []llama.ChatMessage{
		llama.NewChatMessage("system", "You are a funny duck. Tell very short jokes, one or two lines max. Only the joke itself, no introduction or explanation."),
		llama.NewChatMessage("user", "Tell me a quick joke."),
	}
...
func (ai *Instance) MakePrompt(message string) string {
